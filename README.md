...
При първо стартиране се създават **демо акаунти** (ако няма потребител с този имейл):

| Роля | Имейл | Парола |
|------|--------|--------|
| Пътник | `test@example.com` | `password123` |
| Администратор | `admin@example.com` | `password123` |

След вход като админ в менюто се появява **„Админ“** → `/admin` (потребители, блокиране, статистики).

Регистрацията през сайта **не** може да създаде админ — ролята `ROLE_ADMIN` се игнорира и се задава `ROLE_PASSENGER`.
